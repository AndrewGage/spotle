...
async function fetchMysteryArtist() {
    try {
        const response = await fetch('resources/mysteryArtists.json');
        const data = await response.json();

        var randomIndex = Math.floor(Math.random() * data.length);
        var randomArtist = data[randomIndex];

        mysteryArtist = artists[randomArtist.artist.toLowerCase()];
        mysteryArtistSong = randomArtist.song_uri;
        mysteryArtistImage = randomArtist.image_uri;
        mysteryArtistName = randomArtist.artist;
        spotleNumber = randomIndex + 1;

        if (randomIndex > 0) {
            yesterdayMysteryArtist = data[randomIndex - 1].artist;
            yesterdayMysteryArtistImage = artists[data[randomIndex - 1].artist.toLowerCase()].imageUri;
        }

        // Save the mystery artist data to localStorage
        saveGameProgress({
            mysteryArtist: mysteryArtist,
            mysteryArtistSong: mysteryArtistSong,
            mysteryArtistImage: mysteryArtistImage,
            mysteryArtistName: mysteryArtistName,
            spotleNumber: spotleNumber,
            yesterdayMysteryArtist: yesterdayMysteryArtist,
            yesterdayMysteryArtistImage: yesterdayMysteryArtistImage
        });
    } catch (error) {
...
function loadGameProgress() {
    var progressData = localStorage.getItem('gameProgress');
...
// Check if there is saved game progress
var savedProgress = loadGameProgress();

if (!challengeGame && !savedProgress) {
    // Fetch a new mystery artist if it's not a challenge game and there's no saved progress
    await fetchMysteryArtist();
} else if (savedProgress) {
    // Load saved progress if it exists
    mysteryArtist = savedProgress.mysteryArtist;
    mysteryArtistSong = savedProgress.mysteryArtistSong;
    mysteryArtistImage = savedProgress.mysteryArtistImage;
    mysteryArtistName = savedProgress.mysteryArtistName;
    spotleNumber = savedProgress.spotleNumber;
    yesterdayMysteryArtist = savedProgress.yesterdayMysteryArtist;
    yesterdayMysteryArtistImage = savedProgress.yesterdayMysteryArtistImage;

    // Set isInitialLoad to false to indicate that it's no longer the initial load
    isInitialLoad = false;
}

// Use isInitialLoad as needed in the rest of your code
else {
...
function printGuess(guess) {
  ...
  imageNationality.src = 'https://spotle.io/resources/nationalities/' + guess.nationality + '.png';
